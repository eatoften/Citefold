...
| Graph-guided Chat | Pydantic + immutable GraphVersion + deterministic Trace + React defensive parsing | 显式双 Concept 问题附带可解释路径，但不改变 Dense 证据检索 | current-question exact match、完整 selected-Source closure、support basis、反向遍历、Graph context != factual evidence | [chat_graph.py](../../backend/app/chat_graph.py), [ADR-0008](../decisions/ADR-0008-evidence-grounded-concept-graph-and-deterministic-paths.md), [Chat disclosure](../../frontend/src/features/chat/ChatGraphContextDisclosure.tsx) | Draft/M0 |
